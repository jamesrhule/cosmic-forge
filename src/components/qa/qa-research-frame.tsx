      ...
      <ResizablePanel defaultSize={50} minSize={30}>
        <div className="h-full overflow-y-auto p-4">
          <div className="mb-2 flex items-baseline justify-between gap-2">
            <h2 className="text-base font-semibold tracking-tight">ξ × θ scan</h2>
        ...
        <div className="grid h-full grid-rows-2 gap-3 overflow-y-auto p-4">
          {[a, b].map((r, i) => (
            <div key={`${r.id}-${i}`} className="rounded-md border bg-card p-3">
              <div className="mb-2 text-xs font-medium text-muted-foreground">{r.id}</div>
              <SGWBPlot spectra={[{ id: r.id, label: r.id, data: r.spectra.sgwb }]} height={220} />
            </div>
          ))}
        </div>
      </ResizablePanel>
