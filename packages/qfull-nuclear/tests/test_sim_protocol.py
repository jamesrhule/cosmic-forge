...
from qfull_nuc import NuclearSimulation, load_instance
from qfull_nuc.sim import _resolve_path


def test_satisfies_simulation_protocol() -> None:
    qcompass_core = pytest.importorskip("qcompass_core")
    assert isinstance(NuclearSimulation(), qcompass_core.Simulation)


def test_manifest_schema_has_required_keys() -> None:
    schema = NuclearSimulation.manifest_schema()
    for k in ("kind", "backend_preference", "zero_nu_bb_toy"):
...
def test_classical_dispatch_writes_sidecar(tmp_path: Path) -> None:
    qcompass_core = pytest.importorskip("qcompass_core")
    sim = NuclearSimulation(artifacts_root=tmp_path)
    problem = load_instance("zero_nu_bb_l4")
    ...
    assert result.classical_method == "zero_nu_bb_ed"
